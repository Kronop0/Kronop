...
const WalletConnect: React.FC = () => {
  ...
  const getNonce = async (walletAddress: string) => {
    try {
      setIsLoading(true);
      const response = await fetch(`${API_ROOT}/api/auth/nonce?address=${walletAddress}`);
  ...
  const signMessageWithNonce = async () => {
    ...
    try {
      ...
      await signMessage({ account: address as `0x${string}`, message: nonceData.message });
  ...
  const verifySignature = async (walletAddress: string, signature: string, nonceValue: string) => {
    try {
      ...
      const response = await fetch(`${API_ROOT}/api/auth/verify`, {
        method: 'POST',
        headers: {
