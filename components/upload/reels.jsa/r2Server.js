...
const KRONOP_API_URL = API_BASE_URL.replace(/\/+$/, '').replace(/\/api$/, '');
  ...
  uploadPhoto: async (fileUri, fileName, metadata) => {
    try {
      ...
      return {
        ...
        fileName: uniqueFileName,
        publicUrl: `${KRONOP_API_URL}/api/photos/${uniqueFileName}`,
        uploadTime: new Date().toISOString()
      };
  ...
  uploadSong: async (fileUri, fileName, metadata) => {
    try {
      ...
      return {
        ...
        fileName: uniqueFileName,
        publicUrl: `${KRONOP_API_URL}/api/songs/${uniqueFileName}`,
        uploadTime: new Date().toISOString()
      };
  ...
  uploadStory: async (fileUri, fileName, metadata) => {
    try {
      ...
      return {
        ...
        fileName: uniqueFileName,
        publicUrl: `${KRONOP_API_URL}/api/stories/${uniqueFileName}`,
        uploadTime: new Date().toISOString()
      };
