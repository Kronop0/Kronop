...
import { reelsController, ReelItem, ReelStats } from './controller/reels/controller';
...
export default function ReelsToolScreen() {
  ...
  const [summary, setSummary] = useState<ReelStats>({
  ...
  const loadReels = async () => {
    try {
      setLoading(true);
      const reels = await reelsController.getReels();
      setReels(reels);
      const newSummary = reelsController.getStats(reels);
    ...
    return (
      <View style={styles.reelItem}>
        ...
        <View style={styles.reelInfo}>
          <Text style={styles.reelTitle} numberOfLines={1} ellipsizeMode="tail">{item.title}</Text>
