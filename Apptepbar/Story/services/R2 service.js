...
console.log('[KRONOP-DEBUG]   - Account ID:', R2_CONFIG.accountId ? '✅ Loaded' : '❌ Missing');
console.log('[KRONOP-DEBUG]   - Access Key ID:', R2_CONFIG.accessKeyId ? '✅ Loaded' : '❌ Missing');
...
class R2StoryService {
  constructor() {
    ...
    try {
      ...
      console.error('[KRONOP-DEBUG] ❌ Failed to create S3 Client:', error.message);
      throw error;
    }
  }
  ...
  async fetchStories() {
    ...
    try {
      ...
      if (!response.Contents || response.Contents.length === 0) {
        console.warn('[KRONOP-DEBUG] ⚠️ R2 story bucket is empty - no objects found');
        console.warn('[KRONOP-DEBUG] 💡 Please upload story files to the kronop-story bucket');
        return [];
      }

      console.log(`[KRONOP-DEBUG] 📂 Found ${response.Contents.length} objects in bucket, processing...`);
      ...
      const storyFiles = response.Contents.map((item, index) => {
        ...
        const videoExtensions = ['mp4', 'mov', 'avi', 'mkv', 'webm', 'm4v', '3gp', 'flv', 'wmv', 'mpeg', 'mpg'];
        const imageExtensions = ['jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp', 'svg', 'tiff', 'ico', 'heic', 'heif'];
        ...
        if (detectedType === 'unknown') {
          console.log(`[KRONOP-DEBUG] ❌ Skipped unsupported file: ${fileName} (extension: ${fileExtension})`);
          console.log(`[KRONOP-DEBUG] 💡 Supported video formats: ${videoExtensions.join(', ')}`);
          console.log(`[KRONOP-DEBUG] 💡 Supported image formats: ${imageExtensions.join(', ')}`);
          skippedFiles++;
          return null;
        }
        ...
        validStories++;
        console.log(`[KRONOP-DEBUG] ✅ ROBUST Story data created for: ${fileName}`);
        ...
        console.log(`[KRONOP-DEBUG]   - Main URL (NEVER UNDEFINED): ${storyData.url}`);
        console.log(`[KRONOP-DEBUG]   - Fallback URL: ${storyData.fallbackUrl}`);
        console.log(`[KRONOP-DEBUG]   - Image URL: ${storyData.imageUrl || 'undefined (expected for video)'}`);
        console.log(`[KRONOP-DEBUG]   - Video URL: ${storyData.videoUrl || 'undefined (expected for image)'}`);
        console.log(`[KRONOP-DEBUG]   - Thumbnail URL: ${storyData.thumbnailUrl}`);
        console.log(`[KRONOP-DEBUG]   - File size: ${(item.Size || 0) / 1024} KB`);
        
        // Validation check - ensure no undefined critical fields
        if (!storyData.url) {
          console.error(`[KRONOP-DEBUG] ❌ CRITICAL ERROR: Main URL is undefined for ${fileName}`);
          return null;
        }
        
        if (!storyData.type || storyData.type === 'unknown') {
          console.error(`[KRONOP-DEBUG] ❌ CRITICAL ERROR: Type is undefined/unknown for ${fileName}`);
          return null;
        }
      ...
      console.log(`[KRONOP-DEBUG]   - Success rate: ${((validStories / response.Contents.length) * 100).toFixed(1)}%`);
      
      return storyFiles;

    } catch (error) {
      console.error('[KRONOP-DEBUG] ❌ Error fetching stories from R2:', error.message);
  ...
  async fetchStoriesByType(type) {
    ...
    const filteredStories = allStories.filter(story => story.story_type === type);
    console.log(`[KRONOP-DEBUG] 📊 Found ${filteredStories.length} stories of type: ${type}`);
  ...
  async searchStories(query) {
    ...
    console.log(`[KRONOP-DEBUG] 📊 Search results: ${filteredStories.length} stories found`);
    return filteredStories;
  }
  ...
  async getRecentStories(hours = 24) {
    ...
    const recentStories = allStories.filter(story => 
      new Date(story.created_at) > cutoffTime
    );
    
    console.log(`[KRONOP-DEBUG] 📊 Found ${recentStories.length} recent stories out of ${allStories.length} total`);
