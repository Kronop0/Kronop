...
class R2PhotoService {
  ...
  async getPhotosByCategory(category: string, limit: number = 20, offset: number = 0): Promise<Photo[]> {
    console.log(`[r2_service.ts] Photo Fetching Started... Category: ${category} (case-insensitive)`);
  ...
  async getPhotoById(photoId: string): Promise<Photo | null> {
    console.log(`[r2_service.ts] Fetching Single Photo Started... Photo ID: ${photoId}`);
    try {
      ...
      if (!response.ok) {
        console.error(`[r2_service.ts] Failed to fetch photo: ${response.status} - ${response.statusText}, Photo ID: ${photoId}`);
        throw new Error(`Failed to fetch photo: ${response.status}`);
      }

      const photo = await response.json();
      console.log(`[r2_service.ts] Photo Data Received from R2: ${photo ? photo.id : 'null'}`);
      return photo;
    } catch (error) {
  ...
  // Upload photo to R2
  async uploadPhoto(file: File, metadata: Partial<Photo>): Promise<Photo | null> {
    console.log(`[r2_service.ts] Photo Upload Started... File: ${file.name}, Size: ${file.size} bytes`);
    try {
      ...
      if (!response.ok) {
        console.error(`[r2_service.ts] Failed to upload photo: ${response.status} - ${response.statusText}`);
        throw new Error(`Failed to upload photo: ${response.status}`);
      }

      const uploadedPhoto = await response.json();
      console.log(`[r2_service.ts] Photo Uploaded Successfully: ${uploadedPhoto ? uploadedPhoto.id : 'unknown'}`);
  ...
  async getSignedUploadUrl(fileName: string, fileType: string): Promise<string | null> {
    ...
    try {
      ...
      if (!response.ok) {
        console.error(`[r2_service.ts] Failed to get signed URL: ${response.status} - ${response.statusText}`);
        throw new Error(`Failed to get signed URL: ${response.status}`);
      }

      const data = await response.json();
      console.log(`[r2_service.ts] Signed Upload URL Received: ${data.success ? 'Success' : 'Failed'}`);
  ...
  private async getPhotosFromR2Directly(category: string, limit: number): Promise<Photo[]> {
    console.log(`[r2_service.ts] REAL R2 Access: Fetching from bucket: ${R2_CONFIG.R2_BUCKET_NAME}`);
    ...
    try {
      ...
      console.log(`[r2_service.ts] R2 Response received. Contents count: ${response.Contents?.length || 0}`);
      ...
      console.log(`[r2_service.ts] Successfully processed ${validPhotos.length} REAL photos from R2`);
      return validPhotos;

    } catch (error) {
      console.error(`[r2_service.ts] ❌ Error accessing R2 bucket:`, error);
      console.log(`[r2_service.ts] Falling back to empty array - no mock data!`);
      return [];
    }
  }
  ...
  async togglePhotoLike(photoId: string, userId: string): Promise<boolean> {
    console.log(`[r2_service.ts] Toggling Photo Like... Photo ID: ${photoId}, User ID: ${userId}`);
    try {
      ...
      console.log(`[r2_service.ts] Photo Like Toggled: ${success ? 'Success' : 'Failed'}`);
      return success;
    } catch (error) {
  ...
  async addComment(photoId: string, userId: string, comment: string): Promise<boolean> {
    console.log(`[r2_service.ts] Adding Comment to Photo... Photo ID: ${photoId}, User ID: ${userId}, Comment Length: ${comment.length}`);
    try {
      ...
      const success = response.ok;
      console.log(`[r2_service.ts] Comment Added: ${success ? 'Success' : 'Failed'}`);
      return success;
    } catch (error) {
