...
const videosApi = {
  ...
  getVideos: async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.KOYEB_API_URL || process.env.EXPO_PUBLIC_API_URL}/content/videos`);
...
export default function VideoToolScreen() {
  ...
  const loadVideos = async () => {
    ...
    try {
      ...
      const processedVideos = data.map((item: any, index: number) => ({
      ...
      const newSummary = processedVideos.reduce(
        ...
        (acc: { total: number; stars: number; comments: number; shares: number; views: number; duration: string }, video: VideoItem) => {
        ...
        { total: processedVideos.length, stars: 0, comments: 0, shares: 0, views: 0, duration: '0:00' }
